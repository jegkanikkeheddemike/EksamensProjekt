...
public class Building {
    ...
    private void build() {
        ...
        for (int i = 0; i < wallAmount; i++) {
            ...
            if (GameMath.rectDistance(doorSquare[0], doorSquare[1], doorSquare[0] + doorSquare[2],
                    doorSquare[1] + doorSquare[3], newSquare[0], newSquare[1], newSquare[0] + newSquare[2],
                    newSquare[1] + newSquare[3]) > doorWidth) {
                for (Wall wall : interiorWalls) {
                    // The wall should be made if none of the walls are too close, if one wall is
                    // too close it shouldn't be made.
    ...
    void makeRandomZombies(int topleftX, int topleftY, int width, int height) {
        ...
        for (int i = 0; i < amount; i++) {
            Zombie zombie = new Zombie(topleftX + r.nextInt(Math.max(1, width)),
                    topleftY + r.nextInt(Math.max(1, height)), Zombie.randomGenes(), null);
